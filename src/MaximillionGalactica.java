...
public class MaximillionGalactica extends AbstractNegotiationParty {
    ...
    public Action chooseAction(List<Class<? extends Action>> list) {
        ...
        System.out.println(this.getUtilitySpace().getUtility(last) + ">=" + willingness);
        if(this.getUtilitySpace().getUtility(last) >= willingness) return new Accept(this.getPartyId(), last);

        // Offer is no good, propose our own
        return new Offer(this.getPartyId(), new Bid(this.getUtilitySpace().getDomain(), new HashMap<>(proposal)));
    }
